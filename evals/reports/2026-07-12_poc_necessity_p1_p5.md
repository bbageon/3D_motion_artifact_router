...
통과의 구조적 이유(당시 해석): u 축이 "애매하면 약하게"를 가능케 함. **⚠️ 2026-07-22 재해석 (AR-087)**: strength 세분화 ablation 결과 **binary {STOP, u=1.0} 가 graded 정책과 improvement 동률·harm 우위** → 연속 강도 조절은 불필요했고, 정책의 실제 이득은 강도 gradation 이 아니라 **STOP 결정(apply/STOP 라우팅)**에서 나왔다. 따라서 E12 는 "학습된 연속 강도 정책"이 아니라 **"generator/state-조건부 binary 보정 게이트 (u=1.0 또는 STOP)"**로 재서술 (AR-086: u=1.0 이 물리·semantic 최적 + AR-087: binary 충분 → 강도 축이 "u=1.0 고정 + STOP 게이트"로 붕괴). AR-082 의 16/20 은 주로 u=1.0 검증이라 binary 와 정합. 한계: one-shot·MM proxy·b1·δ 조건부. [AR-081](2026-07-13_strength_q_v1_ar081.md)·[AR-082](2026-07-19_ab_policy_result_ar082.md)·[AR-087](2026-07-22_strength_granularity_ablation_ar087.md)

**E12-b — strength 정식 정의 + over-correction 분해 (AR-085/086, 2026-07-22)**: strength `u` = 접지-일관 root 변위 보정의 **적용 분율** (offset(u)=u·offset(1) 선형; u=1 = 접지 추정 기준 제약 포화점). u∈[0,2] 개방(AR-085) 결과 MM-Dist 상 43% 모션이 u>1 에서 최적처럼 보였으나 — **AR-086 물리·semantic 분해로 이것이 MM proxy artifact 로 판정**: foot_skate(물리)와 MM 개선(semantic)이 **둘 다 u=1.0 에서 최적으로 일치**하고, u>1 은 foot_skate 를 악화(발 반대 미끄러짐)시키며 MM 만 개선(tm2t 의 over-translation 편향). ⟹ **평균 고정 strength 최적점 = u=1.0 (물리·semantic 양축)**; u>1 의 MM headroom 은 proxy. AR-077 "over-correction 꼬리 = 물리 harm" 라벨 확정. 확장-grid 정책은 v1 대비 비유의(H-B). 함의: strength 는 정식 정의된 연구 객체; MM proxy 의 over-translation 편향 = 지표 한계 → 지각·foot_skate 병행 필수. [AR-085](2026-07-22_strength_q_v2_open_u_ar085.md)·[AR-086](2026-07-22_u_decomposition_ar086.md)
