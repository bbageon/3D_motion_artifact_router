...
## 2-1. 결론 정정 (2026-07-22 2차 피드백 — 표현 하향)

방어 가능한 가장 정확한 진술 (사용자 리뷰 채택):

> **AR-085 는 기존 [0,1] action space 가 일부 locomotion 모션의 MM-Dist 기준 oracle headroom 을 잘랐음을 확인했다. 그러나 평균 고정 strength 의 최적점은 여전히 u=1.0 이며(mean imp: u=1.0 0.312 > u=1.25 0.279), u>1 의 이득이 물리적 under-correction 복구인지 MM proxy 특성인지는 아직 분리되지 않았다. 확장-grid 정책은 v1 보다 유의하게 개선되지 않았다(H-B).**

즉 §2 의 "u=1 은 ceiling 아님"은 **per-motion oracle-u(MM 기준) 관점**에서만 참이고, **단일 고정 strength 로는 u=1.0 이 여전히 최적**. **⟹ [AR-086 분해 완료](2026-07-22_u_decomposition_ar086.md): u>1 의 MM 이득은 물리 복구가 아니라 MM proxy 선호로 판정** (foot_skate 는 u=1.0 에서 최소, u>1 에서 악화; MM 만 개선). foot_skate·MM 양축 공통 최적 = u=1.0. §2 의 "ceiling 아님" 흥분은 AR-086 으로 하향 — u=1.0 이 물리·semantic 공통 최적이 정답.
