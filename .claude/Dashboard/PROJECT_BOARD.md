# ArtifactRouter — Project Board (index)

> Jira / Plane 스타일 작업 board. **상태별로 파일 분리** (각 state = 한 파일). 본 파일은 **index** — 운영 규약 + state 파일 링크 + epic + 결론 overview. 상세 evidence 는 `reports/` (일지) + `evals/` + git commit 이 single source.

## 상태별 파일 (Plane 스타일 컬럼)

| State | 파일 | 현황 |
|---|---|---|
| 🗂️ Backlog | [backlog.md](backlog.md) | 미착수·미래 (6) |
| 📋 Todo | [todo.md](todo.md) | 착수 준비 (5, 사용자 신호 대기) |
| 🟢 In Progress | [in-progress.md](in-progress.md) | (없음) |
| ✅ Done | [done.md](done.md) | 완료 (AR-001~034) |
| 🚫 Cancelled | [cancelled.md](cancelled.md) | (없음) |

흐름: `Backlog → Todo → In Progress → Done / Cancelled`.

## 사용 규약

- **Issue ID**: `AR-NNN` (append-only 증가, 닫혀도 재사용 금지). **Priority**: 🔴 Urgent / 🟠 High / 🟡 Medium / ⚪ Low. **Epic**: 큰 묶음 (아래 표).
- **작업 시작**: [todo.md](todo.md) 에서 항목을 [in-progress.md](in-progress.md) 로 이동 (한 번에 1개 권장).
- **작업 종료(commit)**: [in-progress.md](in-progress.md) → [done.md](done.md) 이동 + commit 해시/링크 박제. 새 후속 작업은 [backlog.md](backlog.md) 추가.
...
- 상세: [results_draft_g2_stage2.md §7](../../docs/results_draft_g2_stage2.md) · [가설 요약](../docs/hypotheses-summary.md).
