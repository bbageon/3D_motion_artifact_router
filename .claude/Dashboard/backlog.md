...
|---|---|---|---|---|
| AR-078 | RL-Q | **② Routing gate 비교 (held-out)** — generator-only / skate-only(AR-077: benefit-AUC ~0.67 δ조건부) / **richer-state**(AR-065) gate 를 same held-out 에서 비교. 평가지표: benefit-AUC + **harmful-apply@정해진 적용률** (selective prediction, Gangrade AISTATS 2021 — no-harm 은 전체 AUC 보다 이것) | 🔴 | AR-065 선행. per-motion CSV(routing_benefit_permotion) 재사용 — embedding 재계산 불요. **본 결과로 AR-077 마감** |
