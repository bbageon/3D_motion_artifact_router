...
|---|---|---|---|---|---|
| AR-081 | RL-Q | **Strength-Q v1 (사전등록 c342785)** — 한 번의 root 보정 강도 Q(s,u): 13차원 직관 이름 상태 (stepping_speed·body_speed_shortfall·foot_sliding + 신뢰도 2 + 문맥 5 + generator 3) × u∈{0,.25,.5,.75,1} · 그룹 additive 구조 · harm head · Q±gen shortcut 검사 · Markov 축약 검정. one-shot scope (closed-loop 는 별도) | 🔴 | 07-13 | [spec](../docs/dashboard-task-specs/AR-081-strength-q-v1.md) · §5-3 provenance. **1단계 u-grid 라벨 생성 background 실행 중** (preflight: MDM u↑→MM 단조개선 확인) → 완료 후 additive Q 학습·holdout 평가 |
