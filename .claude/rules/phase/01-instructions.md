...
본 프로젝트의 사용자 응답 (메시지) 은 다음 **4 절 구조** 로 작성한다. 사용자가 "어디까지 진행됐고 왜 했는지" 를 한눈에 파악할 수 있게 한다.

```
## 진행사항
- 이번 turn 에서 실제로 수행한·수행 중인 행동 (파일 편집·명령 실행·commit·push 등).
- 짧은 bullet 또는 표.

## 진행이유
- 왜 이 작업을 했는지 — 사용자 directive 또는 직전 결정·게이트 인용.
- 한두 문장.
...
- **코드·경로·CLI 명령·파일명·H-id·W-id** 는 영어 그대로 (변경 금지).
...
3. commit 메시지·파일 내용 등 채팅 외부 산출물.

본 규칙은 모든 phase·skill 응답에 적용된다. 단 본문이 매우 짧을 때 (예: 4 절 모두 한 줄씩 이내) 는 절 제목을 생략하고 한 단락으로 합쳐도 무방.
