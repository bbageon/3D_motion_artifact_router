...
> [`.claude/docs/governance/current_research_position.md`](current_research_position.md) 의 §2 (Evaluator), §3 (NetGain) 도 본 문서와 동기화.
...
| **본 프로젝트 구현** | ✅ 구현 (FidelityLoss Protocol A 의 기반 — [reproducibility-checklist §3-3](../../skills/reproducibility-checklist/SKILL.md)) |
| **분류** | **A. standard** |
...
#### 3-5-1. PenetrateEvaluator (gate) — v0.2.0 (2026-07-07, AR-063)
...
| **본 프로젝트 정의** | foot/ankle joint Y < ground_y - penetrate_eps 의 ratio (per-frame, per-foot). **ground 내부 추정 = feet lower-height 10th percentile** (v0.2.0; coords_protocol 일치). |
| **분류** | **B. variant** |
| **용도** | Layer B gate decision. accept/rollback boundary 의 정량 기준. |
| **Caveat** | **v0.1.0 (min-Y ground) 은 구조적 vacuity — 정의상 fire 불가** ([AR-062 audit A-2](../findings/artifact_tool_alignment_audit.md), 재현 snapshot). v0.1.0 기록의 "관통 0%" 인용 금지. percentile ground 는 **transient** 관통(<10% frames)에만 robust — 지속 관통은 외부 ground_y 필요. clean p99(v2)=0.0404. 유병률(v2, trajectory): occur ≤1.8% — 실측 희소 ([remeasure](../../../evals/snapshots/gate_prevalence_remeasure_ar063_v1.json)). |

#### 3-5-2. FloatEvaluator (gate) — v0.2.0 (2026-07-07, AR-063)

| Item | Value |
|---|---|
| **Source (recent, 2020+)** | **PhysDiff** (Yuan et al. 2023, **ICCV**) — foot floating. **MDM** (Tevet et al. 2023, **ICLR**) — foot contact loss. |
| **본 프로젝트 정의** | contact-intended 추정 frame 중 foot height > float_threshold(0.05) 의 ratio. **contact-intended = (height ≤ 0.10) & (\|수직속도\| ≤ 0.035)** (v0.2.0 — v2 계열, 수평→수직속도 교체). |
| **분류** | **B. variant (gate-form)** |
| **용도** | Layer B gate decision. |
| **Caveat** | v0.1.0 수평속도 contact 은 local 좌표 보행 중 지지발 판정 실패 (AR-062 A-4). clean p99(v2)=0.70 — clean 자체의 float band 비율이 높아 **gate-fire 는 판별력 약함** (생성물 fire ≤1.1%): raw score 사용 원칙. |

#### 3-5-3. SkateEvaluator (gate) — v0.2.0 (2026-07-07, AR-063)
...
| **본 프로젝트 정의** | contact 추정 frame 중 foot 수평변위 > skate_threshold **0.025 m/frame** 의 ratio. **contact = (height ≤ 0.05) & (\|수직속도\| ≤ 0.035)** (v2 — AR-058-3b 일치). |
| **분류** | **B. variant** |
| **용도** | Layer B gate decision. foot sliding artifact 의 정량 (foot floating 과 orthogonal 차원). |
| **Caveat** | **v0.1.0 은 구조적 vacuity** — contact(수평속도≤0.02) ∩ skate(>0.05) = 공집합, contact_labels 없이는 어떤 motion 도 fire 불가 ([AR-062 audit A-1](../findings/artifact_tool_alignment_audit.md)). v0.1.0 의 "skate 0%" (clean p99=0.0 포함) 인용 금지. **좌표 의무: trajectory(world) 입력** — local 은 root 이동 혼입으로 과검출 (실증: motiongpt fire 21.3%(local) vs 1.7%(traj)). clean p99(v2)=0.346; 유병률(v2, traj): **MDM fire 23.1% vs VQ 1.3~1.7%** — foot_skate_world (Cat B) 서열과 수렴. |
...
| **본 프로젝트 구현** | ✅ ([reproducibility-checklist §3-4](../../skills/reproducibility-checklist/SKILL.md)). |
