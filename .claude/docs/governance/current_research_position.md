...
| **핵심 pair — 완주 (2026-07-13 P5 최종화)** | **root progression collapse (MDM) → root-aware correction (AR-072) + generator/state 조건부 적용** | 기전 0.42→1.02 (GT-free) · Cat-A 삼중 유의 개선 (R@1 +0.047·MM −1.0·FID 7.2→3.3, R-Prec 수정 후) · **blind A/B 19/20** · **VQ 는 deficit 없음 + 고정 적용 시 손상 (AR-077, pool-scoped)** — 고정 적용 불가의 직접 근거 | anchoring(증상) 우연 vs root(병인) 압도 = intervention evidence (인과 아님). **per-motion gate 미완 (AR-078 "한계 확정"**: richer state AUC 0.722 vs 0.674 유의 개선하나 harm@30% 감소 비유의; 순위≠안전 축 분리) — "routing 작동" 표현 금지. **b1 마감** (사용자 결정 07-13 — b2 는 논문화 시 재개, pack 준비됨)·MDM·locomotion·단일 벤치마크·δ 조건부. **+07-19 결정 계층 완성 (E12)**: 강도 정책 Q(s,u) 가 정량(holdout +0.061 CI-clean·harmful 1/3)+지각(blind **16/20** p=0.012, 무작위 표본) 이중 통과 — "정책 작동" 유보 해제 (b1, one-shot; VQ 적용 4/8 잔여 약점). 종합 = [P5 최종본](../../../evals/reports/2026-07-12_poc_necessity_p1_p5.md) |
