...
| **Perceptual / visual / motion quality** | NetGain proxy 의 perceptual validity. fidelity 손상의 user-facing 평가. **sub-tier: (a) visual sanity check** (n=수개 sample 의 GIF/PNG, "rank 일치" 확인 — preliminary supports), **(b) perceptual rating** (user study MOS, n≥20-30 pairwise A/B — 정식 quality-validated evidence). | 시각화 단일 sample 결론 ([§3-9](#3-9-단일-sample--단일-trial-결론-금지) 일관). **(a) sub-tier 결과를 "사람 평가로 입증" 으로 인용 금지** — "visual sanity check supports (preliminary, n=X)" 로만 표기 (사용자 directive 2026-05-25 박제). |
